...
#
# Tags look like `v<n8n version>-<release>`:
#
# - if defaults/main.yml points at an n8n version that has never been released,
#   the release counter restarts at 0 (`v2.36.0-0`)
# - otherwise the counter is incremented (`v2.36.0-1`), but only if something
...
# Anchored on `n8n_version:` so that neither `n8n_container_image_tag:` (whose
# value refers to `n8n_version`) nor a commented-out line can be mistaken for it.
version="$(sed -nE 's|^n8n_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the n8n version from $defaults_path"
	exit 1
fi

# The version value carries no leading `v` (e.g. `2.36.0`) while the tags do,
